...
def _leg_half_spread_bps(bid: float, ask: float) -> float:
    """Half-spread of one leg, expressed in bps of the $1 bundle payout.

    The bundle edge is reported as ``(sum_yes_bid - 1) * 10000`` (or symmetric for
    buys), i.e. bps of the $1-per-bundle resolution payout. To subtract leg costs
    in the same units, each leg contributes its absolute half-spread in dollars
    times 10000 — NOT the leg's spread-as-a-fraction-of-its-own-mid. Long-shot
    legs at $0.001 mid have small absolute spreads even when their bps-of-mid
    spread is enormous, and the bundle math cares about the dollar amount.
    """
    if ask <= bid:
        return 0.0
    return (ask - bid) / 2 * 10_000


def _load_watchlist_cids(path: Path) -> set[str]:
...
def scan(
    ...
    for (cid, bucket), legs in by_cond_bucket.items():
        ...
        gross_buy_edge_bps = (1.0 - bundle_ask) * 10_000 - fee_bps
        gross_sell_edge_bps = (bundle_bid - 1.0) * 10_000 - fee_bps
        bundle_leg_cost_bps = (
            _leg_half_spread_bps(yes["bid"], yes["ask"])
            + _leg_half_spread_bps(no["bid"], no["ask"])
        )
        buy_edge_bps = gross_buy_edge_bps - bundle_leg_cost_bps
        sell_edge_bps = gross_sell_edge_bps - bundle_leg_cost_bps
            ...
            opportunities.append(
                {
                    ...
                    "gross_edge_bps": round(gross_buy_edge_bps, 2),
                    "bundle_leg_cost_bps": round(bundle_leg_cost_bps, 2),
                    "yes_ask": yes["ask"],
                    "no_ask": no["ask"],
                }
            )
            ...
            opportunities.append(
                {
                    ...
                    "gross_edge_bps": round(gross_sell_edge_bps, 2),
                    "bundle_leg_cost_bps": round(bundle_leg_cost_bps, 2),
                    "yes_bid": yes["bid"],
                    "no_bid": no["bid"],
                }
...
def scan_championship(
    ...
    for champ, expected in field_counts.items():
        ...
        gross_ask_edges_bps: list[float] = []
        gross_bid_edges_bps: list[float] = []
        leg_cost_bps_samples: list[float] = []
        buy_above = 0
        sell_above = 0
        partial_buckets = 0
        full_buckets = 0
        ...
        for (c, bucket), legs in by_champ_bucket.items():
            ...
            gross_buy_edge_bps = (1.0 - sum_ask) * 10_000 - fee_bps
            gross_sell_edge_bps = (sum_bid - 1.0) * 10_000 - fee_bps
            bundle_leg_cost_bps = sum(
                _leg_half_spread_bps(leg["bid"], leg["ask"]) for leg in legs.values()
            )
            buy_edge_bps = gross_buy_edge_bps - bundle_leg_cost_bps
            sell_edge_bps = gross_sell_edge_bps - bundle_leg_cost_bps
            ask_edges_bps.append(buy_edge_bps)
            bid_edges_bps.append(sell_edge_bps)
            gross_ask_edges_bps.append(gross_buy_edge_bps)
            gross_bid_edges_bps.append(gross_sell_edge_bps)
            leg_cost_bps_samples.append(bundle_leg_cost_bps)
        ...
        per_champ[champ] = {
            ...
            "gross_buy_edge_bps": _quantile_summary(gross_ask_edges_bps),
            "gross_sell_edge_bps": _quantile_summary(gross_bid_edges_bps),
            "bundle_leg_cost_bps": _quantile_summary(leg_cost_bps_samples),
...
def _quantile_summary(values: list[float]) -> dict[str, float | None]:
    if not values:
        return {"count": 0, "median": None, "max": None, "min": None, "p05": None, "p95": None}
    quantiles = statistics.quantiles(values, n=20) if len(values) >= 20 else None
    p05 = quantiles[0] if quantiles else None
...
def main() -> None:
    ...
    p.add_argument("--json-out", type=Path, help="Optional path to write the full JSON report.")
    ...
    p.add_argument(
        ...
        type=Path,
        help=(
            "Filter records to only condition_ids present in this watchlist file. "
            "Useful for excluding historically-collected legs that are no longer in the active universe."
        ),
    )
    args = p.parse_args()

    records, rejected = _load(args.input)
    restricted_cids: set[str] | None = None
    if args.restrict_to_watchlist:
        restricted_cids = _load_watchlist_cids(args.restrict_to_watchlist)
        before = len(records)
        records = [r for r in records if str(r.get("condition_id") or "") in restricted_cids]
        rejected["filtered_off_watchlist"] = before - len(records)
