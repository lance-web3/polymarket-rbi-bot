...
from deploy.scan_live_arb import DEFAULT_CHAMPIONSHIP_PATTERNS
...
def evaluate_championship(scan_row: dict[str, Any], coverage: dict[str, Any]) -> dict[str, Any]:
    ...
    bucket_count = int(scan_row.get("bucket_count") or 0)
    buckets_above_mve = int(scan_row.get("buckets_above_mve") or 0)
...
def main() -> None:
    ...
    rows = scan if isinstance(scan, list) else scan.get("championships", [])
